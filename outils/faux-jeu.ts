...
const COLS_04 = ["rank", "rank_label", "score", "priority",
  "democratic_theme_endorsement", "title", "first_name", "last_name", "commune",
  "department", "insee_code", "endorsement_history", "predecessor",
  "predecessor_mayor", "recent_candidate", "recent_year", "email",
  "phone", "town_hall_hours", "postal_address", "postal_code", "city",
  "website", "contact_form"];

const COLS_01 = ["priority", "score", "democratic_theme_endorsement",
...
const SCORES: Record<string, number> = {
  has_endorsed: 3, commune_has_endorsed: 0, no_signal: 0,
};
...
const DEPARTMENTS = ["Aveyron", "Cantal", "Moselle", "Nord", "Bas-Rhin",
  "Martinique", "Polynésie française", "Vosges", "Somme", "Pas-De-Calais"];

const COMMUNE_SHAPES = [
  "Sainte-Fiction", "Havange-Fictive", "Ambléon-Fictif", "Le Fictif",
  "La Fiction", "Les Fictions", "L'Haÿ-Fictive", "Œting-Fictif",
  "Fiction-sur-Mer", "Hiva-Fictive",
];
...
function row(i: number, rank: string): Record<string, string> {
  ...
  const overseas = department === "Martinique" || department === "Polynésie française";
  return {
    rank, rank_label: RANKS[rank],
    ...
    democratic_theme_endorsement: rank === "has_endorsed" && i % 5 === 0 ? "oui" : "",
    title: woman ? "Mme" : "M.",
    ...
    insee_code: overseas
      ? `97${zeros((Math.floor(i / DEPARTMENTS.length) * 2
        + (department === "Martinique" ? 0 : 1)) % 1000, 3)}`
      : `9${zeros(i % 7000, 4)}`,
    ...
    email: i % 23 === 0 ? ""
      : i % 17 === 0 ? `mairie${i}@exemple-fictif.fr;secretariat${i}@exemple-fictif.fr`
    ...
    postal_code: `1${zeros(i, 4)}`, city: `Sainte-Fiction-${i}`,
    website: "", contact_form: "",
  };
}
...
export function main(dest: string): void {
  ...
  for (const [rank, count] of [["has_endorsed", 260], ["commune_has_endorsed", 180],
    ["no_signal", 620]] as [string, number][]) {
    for (let n = 0; n < count; n++) rows.push(row(++i, rank));
  }
  writeFileSync(join(dest, "04_base_complete.csv"), writeCsv(COLS_04, rows), "utf8");

  // file 01 (mass mailing) only contains the endorsers
  const endorsers = rows.filter((m) => m.rank === "has_endorsed").map((m) => ({
  ...
  writeFileSync(join(dest, "01_maires_cibles_prioritaires.csv"),
    writeCsv(COLS_01, endorsers), "utf8");
