...
const backup = (extra: Partial<Backup> = {}): Backup => ({
  format: "paraphe/1", exported_at: "2026-08-13T00:00:00Z",
  mayors: [], tracking: [], settings: [], ...extra,
});
...
describe("the import", () => {
  it("refuses a file that is not a paraphe export", async () => {
    await expect(DB.importAll({ format: "autre" } as Backup)).rejects
      .toThrow(/pas un export paraphe/);
  });
  ...
  it("keeps existing work when merging a damaged file", async () => {
    ...
    const report = await DB.importAll(backup({
  ...
  it("does not fail in the middle of an overwrite", async () => {
    ...
    const report = await DB.importAll(backup({
      tracking: [
        { insee_code: "01033", status: "refused", updated_at: "2026-08-13 à 10:00", notes: [] },
  ...
  it("does not touch the configuration when merging", async () => {
    ...
    expect(await DB.readSetting("campagne", null)).toEqual({ candidat: "Camille Réel" });
  ...
  it("takes the settings it does not hold, and keeps those it does", async () => {
    ...
    expect(await DB.readSetting("campagne", null))
      .toEqual({ candidat: "Ariane Fictive" });
  ...
  it("keeps the most recent version on merge", async () => {
    ...
    const report = await DB.importAll(backup({
      tracking: [{
        insee_code: "01022", status: "refused", updated_at: "2020-01-01 à 00:00",
        notes: [{ ts: "2020-01-01 à 00:00", status: "refused", note: "vieux" }],
      }],
    }), { merge: true });
