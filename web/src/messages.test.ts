...
const base = (extra: Mayor = {}): Mayor => ({
  ...
  recent_candidate: "",
  recent_year: "",
  email: "mairie@exemple-fictif.fr",
...
const COMMUNE = base({
  ...
  predecessor: "Claude ANCIEN",
  predecessor_mayor: "oui",
  endorsement_history: "2017: EXEMPLE Alex (A)",
});

const CLAIMS =
  /vous avez présenté|votre présentation de \d{4}|votre parrainage|vous en remercier|ce geste/i;

describe("the central invariant", () => {
  it.each([
    ["commune_has_endorsed", COMMUNE],
    ["no_signal", base()],
...
describe("rendering hygiene", () => {
  ...
  it("follows the civility for the salutation", () => {
    expect(
      email(base({ title: "Mme", first_name: "Camille" }), CFG).body,
  ...
  it("refuses a civility outside the domain", () => {
    for (const bad of ["Madame", "Mlle", "", "Dr"]) {
      expect(() => email(base({ title: bad }), CFG)).toThrow(MissingField);
    }
  });
});
...
describe("directory data", () => {
  ...
  it("splits concatenated addresses", () => {
    const { valid, rejected } = emailAddresses({
      email: "a@x.fr;b@y.fr,c@z.fr",
    });
  ...
  it("flags an undeliverable letter", () => {
    ...
    expect(
      incompleteAddress({ postal_code: "12000", commune: "X" }),
    ).toBeNull();
  });
});
...
describe("formatting", () => {
  it("groups the years of the same candidate", () => {
    expect(
      readableHistory(
        "2017: ARTHAUD Nathalie (B) | 2022: ARTHAUD Nathalie (B)",
      ),
