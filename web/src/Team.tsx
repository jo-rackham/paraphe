...
import {
  Alerte, CAMPAIGN_FIELDS, campaignLabel, Chip, Fiche, Guide, Hexagone, label,
  PiedDePage, RenderGuard, ROLES, type CardDraft,
  STATUSES,
} from "./common.tsx";
import type {
  Account, Dashboard as DashboardData, Facets, MayorCard, Me, Message,
  ServerConfig, TeamData,
} from "./types.ts";
...
export default function Team({ config }: { config: ServerConfig }) {
  ...
  useEffect(() => {
    const lost = () => {
      ...
      setMessage({
        tone: "erreur",
        text: "Votre session a expiré. Reconnectez-vous — le travail déjà "
  ...
  if (!me) {
    return (
      <Coquille cfg={cfg}>
        ...
        <Connexion cfg={cfg} onSignedIn={async (m: Me) => {
          ...
          if (lastAccount.current && lastAccount.current !== m.account.email) {
            cardDrafts.current = {};
          }
  ...
  return (
    <Coquille cfg={cfg} me={me} tab={tab} setTab={setTab}
      onSignOut={signOut}>
      <Alerte message={message} onClose={() => setMessage(null)} />
      {cfg.unfilled?.length > 0 && (
        <p className="alerte">
          <strong>Campagne non configurée.</strong> Les messages contiennent
          encore des valeurs d'exemple ({cfg.unfilled.map(campaignLabel).join(", ")}) :{" "}
      ...
      {tab === "tableau" && (
        <Tableau cfg={cfg} me={me} onError={report} onOpen={openCard}
          onMessage={setMessage} />
      )}

      {tab === "maires" && (
        <ListeServeur onError={report} onOpen={openCard} />
      )}

      {tab === "fiche" && chosen && (
        <Fiche
          mayor={chosen.mayor} cfg={cfg.campaign} personalNote={account.personal_note}
          signer={account.name} drafts={cardDrafts}
          status={chosen.mayor.status} notes={chosen.notes ?? []}
          onBack={() => setTab("maires")}
          header={<ReserveePar mayor={chosen.mayor} me={account} />}
          onStatus={async (status: string, note: string) => {
            const fresh = await API.setStatus(chosen.mayor.insee_code, status, note);
            setChosen(fresh);
          }}
        />
      )}
      ...
      {tab === "equipe" && (
        <GestionEquipe onError={report} me={me} cfg={cfg}
          onCfg={setCfg} onMessage={setMessage} />
      )}

      {tab === "profil" && (
        <Profil me={me} cfg={cfg} onError={report}
          onSaved={(personalNote: string) => {
            setMe((m) => (m
              ? { ...m, account: { ...m.account, personal_note: personalNote } }
              : m));
            setMessage({ tone: "ok", text: "Votre touche personnelle est enregistrée." });
          }} />
      )}
    </Coquille>
  );
...
function Coquille({ cfg, me, tab, setTab, onSignOut, children }: CoquilleProps) {
    ...
    <>
      ...
      <header>
        <span className="marque">
          <Hexagone />
          <span>paraphe<br /><span className="sous">
            {cfg.campaign?.candidat}</span></span>
        </span>
        {me && tabs.map(([key, label]) => (
          <button key={key} className="lien"
            style={tab === key ? { background: "var(--entete-hover)" } : undefined}
        ...
        {me && (
          <span className="qui">
            ...
            {" · "}
            <button className="lien" onClick={onSignOut}>déconnexion</button>
          </span>
        )}
      </header>
      <div className="rayures" />
      <RenderGuard><main>{children}</main></RenderGuard>
...
function Connexion(
  { cfg, onSignedIn }: { cfg: ServerConfig; onSignedIn: (m: Me) => void },
) {
  ...
  return (
    <>
      ...
      <div className="carte">
        ...
        <p className="gris">
          {data.batch_size} maires vous sont réservés, les mieux notés
          d'abord. Personne d'autre ne les recevra.
        </p>
        <form className="enligne" onSubmit={(e) => { e.preventDefault(); take(); }}>
          <div>
            <label>Département
          ...
          <div>
            <label>
              <input type="checkbox" checked={democracy}
      ...
      {data.team.length > 0 && (
        <>
          ...
          <div className="carte">
            <table>
              <thead><tr><th>Bénévole</th><th>Fiches</th><th>Contactées</th></tr></thead>
              <tbody>
                {data.team.map((e) => (
                  <tr key={e.who}>
                    <td>{e.who}{e.who === me.account.name ? " (vous)" : ""}</td>
                    <td>{e.n}</td><td>{e.done}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function LigneMaire(
  { m, onOpen }: { m: MayorCard; onOpen: (insee: string) => void },
) {
  return (
    <tr>
      <td>
        <button className="lien" onClick={() => onOpen(m.insee_code)}>
      ...
      <td>
        ...
        {m.volunteer_name && <><br /><span className="gris">{m.volunteer_name}</span></>}
      </td>
    </tr>
  );
}
...
function ListeServeur(
  { onError, onOpen }: { onError: (e: unknown) => void; onOpen: (i: string) => void },
) {
  ...
  const [facets, setFacets] = useState<Facets>({ departments: [], by_rank: {} });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setRows(r.rows); setTotal(r.total); setNext(r.next); setFailed(null);
      } catch (e) {
  ...
  return (
    <>
          ...
          <label>Vivier
            <select value={rank} onChange={(e) => setRank(e.target.value)}>
              {Object.entries(M.RANKS).map(([k, v]) => (
                <option key={k} value={k}>{v} ({facets.by_rank[k] ?? 0})</option>
              ))}
            </select>
          </label>
        </div>
      ...
      <p className="gris">{rows.length} affiché(s) sur {total}.</p>
      <div className="carte">
        <table>
          <thead>
            <tr><th>Commune</th><th>Département</th><th>Signal</th><th>Statut</th></tr>
        ...
        <div ref={sentinel} />
        {loading && <p className="gris" style={{ textAlign: "center" }}>Chargement…</p>}
        {failed !== null && (
          <p className="alerte" style={{ textAlign: "center" }}>
            Chargement interrompu.{" "}
            <button className="lien" onClick={() => {
              setFailed(null);
              API.mayors(JSON.parse(key)).then((r) => {
                setRows(r.rows); setTotal(r.total); setNext(r.next);
              }).catch(onError);
            }}>Réessayer</button>
          </p>
        )}
        {failed === null && next === null && rows.length > 0 && (
          <p className="gris" style={{ textAlign: "center" }}>— fin de la liste —</p>
        )}
...
function ConfigurationCampagne(
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onCfg({
        ...cfg, campaign: r.campaign, batch_size: r.batch_size,
        unfilled: r.unfilled,
      });
      onMessage({
        tone: "ok",
        text: r.unfilled.length === 0
  ...
  return (
    <form className="carte" onSubmit={save}>
      ...
      <p className="gris">
        ...
        manque, l'application le dit sur chaque page et le publipostage de
        masse refuse de tourner.
      </p>
      ...
      <p><label>Maires attribués par « prendre un lot »
...
function GestionEquipe(
  { onError, me, cfg, onCfg, onMessage }: {
    onError: (e: unknown) => void;
    me: Me;
    cfg: ServerConfig;
    onCfg: (c: ServerConfig) => void;
  ...
  const [created, setCreated] = useState<
    { email: string; name: string; role: string; password: string } | null>(null);
  const [draft, setDraft] = useState({ email: "", name: "", role: "volunteer", team_id: "" });
  ...
  const createAccount = async (e: React.FormEvent) => {
    ...
    try {
      const r = await API.createAccount({
        ...
        team_id: coordination && draft.team_id
          ? Number(draft.team_id) : undefined,
      });
      setCreated(r);
  ...
  return (
    <>
      ...
      {coordination && (
        <ConfigurationCampagne cfg={cfg} onCfg={onCfg} onError={onError}
          onMessage={onMessage} />
      )}
      ...
      {created && (
        <div className="carte alerte">
          <p>
            <strong>Accès créé pour {created.name} ({created.email}).</strong>
          </p>
          <p>Mot de passe provisoire — <strong>affiché une seule fois</strong>,
            à transmettre de vive voix :</p>
          <p className="grand-tel">{created.password}</p>
          <button className="lien" onClick={() => setCreated(null)}>j'ai noté</button>
        </div>
      )}
      ...
      <div className="carte">
        ...
        <form className="enligne" onSubmit={createAccount}>
          ...
          <div><label>Adresse email
            <input type="text" value={draft.email}
      ...
      <div className="carte">
        <table>
          <thead>
            <tr><th>Nom</th><th>Adresse</th><th>Rôle</th><th>Équipe</th><th /></tr>
          </thead>
          <tbody>
            {data.accounts.map((c) => (
              <tr key={c.email} style={c.active ? undefined : { opacity: 0.5 }}>
                ...
                <td>
                  {c.email !== me.account.email && (
                    <button className="lien" onClick={async () => {
                      try {
                        await API.toggleAccount(c.email);
                        await reload();
                      } catch (e) { onError(e); }
                    }}>{c.active ? "désactiver" : "réactiver"}</button>
                  )}
                </td>
              </tr>
            ))}
      ...
      {coordination && (
        <>
          ...
          <div className="carte">
            <form className="enligne" onSubmit={async (e) => {
              e.preventDefault();
              try {
              ...
              <div><label>Nom de l'équipe
                <input type="text" value={team.name}
                  onChange={(e) => setTeam({ ...team, name: e.target.value })} />
              </label></div>
              <div><label>Départements (plusieurs possibles)
                <select multiple size={5} value={team.departments}
                  onChange={(e) => setTeam({
                    ...team,
                    departments: [...e.target.selectedOptions].map((o) => o.value),
                  })}>
            ...
            <table>
              <thead>
                <tr><th>Équipe</th><th>Départements</th><th>Membres</th><th>Fiches</th></tr>
              </thead>
              <tbody>
                {data.teams.map((g) => (
                  <tr key={g.id}>
                    <td>{g.name}</td>
                    <td className="gris">{g.departments || "toute la France"}</td>
                    <td>{g.members}</td><td>{g.reserved}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function Profil({ me, cfg, onError, onSaved }: ProfilProps) {
  const [personalNote, setPersonalNote] = useState(me.account.personal_note ?? "");
  const [sending, setSending] = useState(false);
  return (
    <>
      <h1>Mon profil</h1>
      <div className="carte">
        <p>
          <strong>{me.account.name}</strong> — {me.account.email}<br />
          <span className="gris">
            Rôle : {label(ROLES, me.account.role)} · Équipe : {me.account.team_name ?? "nationale"}
      ...
      <div className="carte">
        ...
        <p><textarea rows={4} value={personalNote}
          onChange={(e) => setPersonalNote(e.target.value)} /></p>
        ...
        }}>{sending ? "Enregistrement…" : "Enregistrer"}</button>
      </div>

      <div className="carte">
        <h2 style={{ marginTop: 0 }}>La campagne</h2>
        <p className="gris">
          Ces valeurs remplissent tous les messages. Seule la coordination
          peut les changer, dans l'onglet « Mon équipe ».
        </p>
        <table>
          <tbody>
