...
test.describe.serial("local teams", () => {
  let leadPassword = "";
  ...
  test("coordination draws a team and its perimeter", async ({ page }) => {
    ...
    await page.getByLabel("Départements (plusieurs possibles)")
  ...
  test("coordination opens a lead access inside that team", async ({ page }) => {
    await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
    await openTab(page, "Mon équipe");
    ...
    await page.getByRole("combobox", { name: "Équipe", exact: true })
  ...
  test("the lead opens volunteer accesses, without choosing roles",
    async ({ page }) => {
      ...
      await expect(page.getByText("vous ouvrez des accès bénévoles")).toBeVisible();
  ...
  test("a team only draws from its own departments", async ({ page }) => {
    ...
    const board = await (await page.request.get(`${ORIGIN}/api/dashboard`)).json();
    expect(board.mine.length).toBeGreaterThan(0);
    for (const m of board.mine) {
      expect(m.department, `${m.insee_code} drawn outside the perimeter`)
        .toBe("Aveyron");
    }
  ...
  test("a card reserved outside the team is refused, not just hidden",
    async ({ page, playwright }) => {
      ...
      const board = await (await coordination.get(`${ORIGIN}/api/dashboard`)).json();
      ...
      expect((await card.json()).error).toContain("réservée par une autre équipe");
    });

  test("a deactivated access no longer opens", async ({ page }) => {
    await signIn(page, ORIGIN, LEAD.email, leadPassword);
    await openTab(page, "Mon équipe");
    await page.locator("tr", { hasText: VOLUNTEER.email })
      .getByRole("button", { name: "désactiver" }).click();
    await expect(page.locator("tr", { hasText: VOLUNTEER.email }))
    ...
    await page.getByLabel("Adresse email").fill(VOLUNTEER.email);
    await expect(page.getByLabel("Adresse email")).toHaveValue(VOLUNTEER.email);
