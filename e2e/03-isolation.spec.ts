...
test.describe
  .serial("two campaigns side by side", () => {
    let password = "";
    ...
    test("a second campaign is opened through the API", async ({ page }) => {
      ...
      const filed = await page.request.post(`${API_ORIGIN}/api/request`, {
        data: {
          slug: SLUG,
          name: "Campagne Troisième",
          requester_email: REQUESTER,
          requester_name: "Autre Porteur",
          message: "une autre candidature",
        },
      });
      ...
      const queue = await (
        await page.request.get(`${API_ORIGIN}/api/admin/requests`)
      ).json();
      const pending = queue.requests.find(
        (d: { slug: string; state: string }) =>
          d.slug === SLUG && d.state === "pending",
      );
      expect(
        pending,
        "the request is not in the moderation queue",
      ).toBeTruthy();
    ...
    test("what one campaign reserves stays invisible to the other", async ({
      page,
    }) => {
      ...
      const mine = await (
        await page.request.get(`${campaignOrigin(SLUG)}/api/dashboard`)
      ).json();
      ...
      for (const insee of reserved) {
        ...
        expect(card.status(), `${insee} refused to the other campaign`).toBe(
          200,
        );
        const { mayor, notes } = await card.json();
        expect(
          mayor.volunteer,
          `${insee} carries the other campaign's volunteer`,
        ).not.toBe(REQUESTER);
      ...
      const csv = await (
        await page.request.get(
          `${campaignOrigin(FIRST_CAMPAIGN)}/api/export.csv`,
        )
      ).text();
      expect(csv).not.toContain(REQUESTER);
    });

    test("a session does not cross from one campaign to the other", async ({
      page,
    }) => {
      await signIn(page, campaignOrigin(SLUG), REQUESTER, password);
    ...
    test("an unknown subdomain is not served some campaign at random", async ({
      page,
    }) => {
