...
test.describe.serial("browser mode", () => {
  ...
  test("what is recorded shows on the list it came from", async ({ page }) => {
    await page.goto(`${STATIC_ORIGIN}/`);
    const first = page.locator("table button.lien").first();
    await expect(first).toBeVisible({ timeout: 20_000 });
    const town = (await first.innerText()).trim();
  ...
  test("never claims to be the team application", async ({ page }) => {
    ...
    await expect(page.getByText("aucune donnée ne quitte ce navigateur"))
      .toBeVisible();
  });

  test("the department filter narrows the list to one department",
    async ({ page }) => {
      await page.goto(`${STATIC_ORIGIN}/`);
      await expect(page.locator("table button.lien").first())
        .toBeVisible({ timeout: 20_000 });

      const others = (await page.getByLabel("Département").locator("option")
        .allInnerTexts()).filter((d) => !d.includes("Aveyron") && !d.includes("—"));
  ...
  test("the campaign filled in the browser feeds the messages",
    async ({ page }) => {
      await page.goto(`${STATIC_ORIGIN}/`);
      await expect(page.locator("table button.lien").first())
        .toBeVisible({ timeout: 20_000 });
      ...
      await page.getByRole("button", { name: "Enregistrer" }).click();
      await expect(page.getByText("Campagne enregistrée dans ce navigateur."))
        .toBeVisible();
  ...
  test("a backup leaves, the browser is wiped, the backup comes back whole",
    async ({ page }) => {
      await page.goto(`${STATIC_ORIGIN}/`);
      const first = page.locator("table button.lien").first();
      await expect(first).toBeVisible({ timeout: 20_000 });
      ...
      await expect(page.getByText("Tout a été effacé de ce navigateur."))
        .toBeVisible();
      await page.getByRole("button", { name: "Les maires" }).click();
