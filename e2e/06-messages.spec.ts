...
test.describe.serial("the written messages", () => {
  test("the volunteer's personal touch enters their emails", async ({ page }) => {
    await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
    await openTab(page, "Mon profil");

    await page.locator(".carte", { hasText: "Votre touche personnelle" })
      .locator("textarea").fill(PERSONAL);
    await page.locator(".carte", { hasText: "Votre touche personnelle" })
      .getByRole("button", { name: "Enregistrer" }).click();
    await expect(page.getByText("Votre touche personnelle est enregistrée."))
      .toBeVisible();
  ...
  // sender's.
  test("the email is signed by whoever is actually sending it",
    async ({ page }) => {
      await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
      await openTab(page, "Mon tableau");
      await page.locator("table button.lien").first().click();
  ...
  test("a mayor who endorsed is thanked, by name and year", async ({ page }) => {
    await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
    await openTab(page, "Mon tableau");
    await page.locator("table button.lien").first().click();
  ...
  test("outside the priority pool, the message switches to discovery",
    async ({ page }) => {
      await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
      ...
      await expect(page.getByText("Vous êtes sorti du vivier prioritaire"))
        .toBeVisible();
  ...
  test("what the volunteer rewrites is what the mail client receives",
    async ({ page }) => {
      ...
      const href = await page.getByRole("link", { name: /Ouvrir dans ma messagerie/ })
        .getAttribute("href");
      ...
      expect(href).toMatch(new RegExp(`^mailto:${encodeURIComponent(address)}\\?`));
    });
  ...
  test("a rewritten email survives a look at the Guide, and the touch reaches it",
    async ({ page }) => {
      await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
      ...
      expect(await page.getByLabel("Note").inputValue()).toBe("il rappelle jeudi");
  ...
  test("the letter and the call script are ready to use", async ({ page }) => {
    await signIn(page, ORIGIN, COORDINATION.email, COORDINATION.password);
    await openTab(page, "Mon tableau");
    await page.locator("table button.lien").first().click();
    ...
    const script = await page.locator("details", { hasText: "Téléphone" })
